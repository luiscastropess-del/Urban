...
import { getPlace, updatePlace } from "@/app/actions";
...
export default function EditPlacePage() {
    ...
    if (id) {
      getPlace(id).then(data => {
        if (data) {
          ...
          try {
             if (data.images) parsedImages = JSON.parse(data.images);
          } catch(err) {}
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('⚠️ Erro ao atualizar o local');
    } finally {
      setLoading(false);
    }
  };
      ...
      <header className="px-5 pt-6 pb-3 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-10 shrink-0">
        <div className="flex items-center gap-3">
          <button
            ...
            className="h-10 w-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0"
          >
            <ArrowLeft className="text-slate-600 dark:text-slate-300" size={20} />
          </button>
          <h1 className="text-xl font-bold">{step === 1 ? 'Editar local' : step === 2 ? 'Editar fotos' : 'Revisar Edição'}</h1>
        </div>
      </header>
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 rounded-2xl p-4 mb-5 border border-green-200 dark:border-green-800/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-green-700 dark:text-green-400">Tudo certo! 🎉</p>
                  <p className="text-xs text-green-600 dark:text-green-500 mt-0.5">Revise as informações e salve as atualizações do local.</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/70 dark:bg-slate-800/70 border border-white/20 rounded-2xl p-4 mb-5 shadow-sm">
              <h3 className="font-bold text-sm mb-4 flex items-center gap-2">
                <Rocket size={16} className="text-orange-500" /> Plano selecionado
              </h3>
            ...
            <div className="flex gap-3 mb-6">
              ...
              <button onClick={submitForm} disabled={loading} className="flex-[2] py-3 bg-gradient-to-r from-orange-500 to-green-500 text-white rounded-xl font-semibold text-sm shadow-md shadow-orange-500/20 disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? 'Salvando...' : <><Save size={16} /> Salvar Edições</>}
              </button>
            </div>
